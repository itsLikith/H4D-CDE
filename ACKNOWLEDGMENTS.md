<div align="center">
  <h1>Acknowledgments</h1>
  <h3>PRISM Repository Implementation</h3>

  <p>
    <img src="https://img.shields.io/badge/Based%20on-Research%20Paper-0A66C2.svg?style=for-the-badge" alt="Research Paper">
    <img src="https://img.shields.io/badge/Conference-ICSPIS%202025-blue.svg?style=for-the-badge" alt="Conference">
    <img src="https://img.shields.io/badge/License-Apache%202.0-blue.svg?style=for-the-badge" alt="License">
  </p>
</div>

<br>

## Research Foundation

This repository implements the methods, architecture, and findings presented in the following peer-reviewed research paper:

> **AI-Augmented Hexagonal Voxelization for Scalable Conflict Detection in Urban Air Mobility**

**Authors**  
Dr. Deepudev Sahadevan¹, Dr. Hannah Al Ali¹, and Dr. Chilka Mahesh²

¹ Faculty of Mathematics and Data Science, Emirates Aviation University, Dubai, UAE  
² CNS Department, Airports Authority of India, Kalaburagi Airport, India

**Conference**  
2025 8th International Conference on Signal Processing and Information Security (ICSPIS)

**DOI**  
[10.1109/ICSPIS67605.2025.11318403](https://doi.org/10.1109/ICSPIS67605.2025.11318403)

---

## Institutional Support

The original research was supported by:

- **Emirates Aviation University** — for providing the necessary facilities and academic environment to conduct this research.

We gratefully acknowledge the contributions of the authors and the supporting institution that made both the research and this open implementation possible.

---

## Citation

If you use this repository or the underlying methods in your work, please cite the original paper:

```bibtex
@inproceedings{sahadevan2025ai,
  title     = {AI-Augmented Hexagonal Voxelization for Scalable Conflict Detection in Urban Air Mobility},
  author    = {Sahadevan, Deepudev and Al Ali, Hannah and Mahesh, Chilka},
  booktitle = {2025 8th International Conference on Signal Processing and Information Security (ICSPIS)},
  year      = {2025},
  doi       = {10.1109/ICSPIS67605.2025.11318403}
}
```

---

## Disclaimer

This software repository is an independent implementation of the concepts and methods described in the above paper.  
It is provided for research, educational, and development purposes under the Apache License 2.0.

<br>


<div align="center">
  <sub>
    <strong>HIVE</strong> — Hexagonal Indexing and Voxelization Engine<br>
    Built upon peer-reviewed research in Urban Air Mobility conflict detection
  </sub>
</div>