...
def generate_synthetic_occupancy_series(
    ...
    for _ in range(n_series):
        ...
        bursts = rng.poisson(lam=0.1, size=total_steps) * rng.uniform(
            1.0, 2.0, size=total_steps
        )
...
def train_demand_forecaster(
    ...
    with torch.no_grad():
        ...
        mape_pct = float(
            np.mean(np.abs((actual - predicted) / np.maximum(actual, 1.0))) * 100.0
        )

    print(
        f"[*] Demand Forecaster Validation MAPE: {mape_pct:.2f}% (Paper Target: ≤ {PAPER_BENCHMARK_MAPE_PCT}%)"
    )
